...
function deadlineString() {
  return new Date(Date.now() + 8 * 86400000).toISOString().slice(0, 16);
}

test.describe('Full Procurement Lifecycle', () => {
  for (const requestType of ['RFI', 'RFP', 'RFQ'] as RequestType[]) {
    test(`complete ${requestType} lifecycle end-to-end`, async ({ page }) => {
      test.setTimeout(300000);
      const TITLE = `E2E ${requestType} Lifecycle - ${Date.now()}`;
      let procId = '';

      // ════════════════════════════════════════════════════════════════
      // Step 1: Requester creates and submits procurement
      // ════════════════════════════════════════════════════════════════
      await login(page, 'requester@ebidding.com');
      await page.goto('/procurements/new');
      await page.waitForLoadState('networkidle');
      await page.getByText(requestType).first().click();
      await page.getByRole('button', { name: 'Continue' }).click();
      await page.getByLabel('Request Title').fill(TITLE);
      await page.getByLabel('Description').fill(`E2E test ${requestType} full lifecycle`);
      await page.getByRole('button', { name: 'Continue' }).click();
      ...
      // ════════════════════════════════════════════════════════════════
      // Step 2: Procurement reviews → approves → publishes (type-specific)
      // ════════════════════════════════════════════════════════════════
      ...
      // Type-specific publish path
      if (requestType === 'RFI') {
        // RFI: Publish → RFI Published → Start Collection → Close → Draft RFP → Publish RFP
        await page.getByRole('button', { name: 'Publish' }).click();
        await page.getByRole('dialog').getByLabel('Submission Deadline').fill(deadlineString());
        await page.getByRole('dialog').getByRole('button', { name: 'Publish' }).click();
        await page.waitForLoadState('networkidle');
        await expect(page.getByText('RFI Published').first()).toBeVisible({ timeout: 10000 });

        await page.getByRole('button', { name: 'Start Collecting Responses' }).click();
        await page.waitForLoadState('networkidle');
        await expect(page.getByText('RFI Collecting').first()).toBeVisible({ timeout: 10000 });

        await page.getByRole('button', { name: 'Close RFI Collection' }).click();
        await page.waitForLoadState('networkidle');
        await expect(page.getByText('RFI Closed').first()).toBeVisible({ timeout: 10000 });

        await page.getByRole('button', { name: 'Draft RFP' }).click();
        await page.waitForLoadState('networkidle');
        await expect(page.getByText('RFP Drafting').first()).toBeVisible({ timeout: 10000 });

        await page.getByRole('button', { name: 'Publish RFP' }).click();
        await page.waitForLoadState('networkidle');
        await expect(page.getByText('RFP Published').first()).toBeVisible({ timeout: 10000 });
      } else if (requestType === 'RFP') {
        // RFP: Publish → RFP Published
        await page.getByRole('button', { name: 'Publish' }).click();
        await page.getByRole('dialog').getByLabel('Submission Deadline').fill(deadlineString());
        await page.getByRole('dialog').getByRole('button', { name: 'Publish' }).click();
        await page.waitForLoadState('networkidle');
      ...
      // ════════════════════════════════════════════════════════════════
      // Step 5: Evaluation — criteria → scoring → consolidation
      // ════════════════════════════════════════════════════════════════
