...
describe('ProcurementsService', () => {
  ...
  const mockUser = { id: 'user-1', email: 'test@test.com', fullName: 'Test', role: 'REQUESTER' };
  ...
  describe('create', () => {
    it('should create a procurement with generated request number', async () => {
      prisma.user.findUnique.mockResolvedValue({ id: 'user-1', propertyId: 'prop-1' });
    ...
    it('should auto-assign user property if not provided', async () => {
      prisma.user.findUnique.mockResolvedValue({ id: 'user-1', propertyId: 'prop-1' });
      prisma.procurement.create.mockImplementation(({ data }: any) => Promise.resolve({ ...mockProcurement, ...data }));
      prisma.file.updateMany.mockResolvedValue({ count: 0 });
  ...
  describe('findAll', () => {
    beforeEach(() => {
      prisma.user.findUnique.mockResolvedValue({ id: 'user-1', propertyId: null });
      prisma.procurement.findMany.mockResolvedValue([mockProcurement]);
      prisma.procurement.count.mockResolvedValue(1);
      prisma.procurement.groupBy.mockResolvedValue([{ status: 'DRAFT', _count: { status: 1 } }]);
    });
    ...
    it('should filter by status and requestType', async () => {
      await service.findAll({ status: 'DRAFT', requestType: 'RFP' } as any, mockUser);
      expect(prisma.procurement.findMany).toHaveBeenCalled();
    });

    it('should filter by search term', async () => {
      await service.findAll({ search: 'test' } as any, mockUser);
    });

    it('should filter by date range', async () => {
      await service.findAll({ dateFrom: '2026-01-01', dateTo: '2026-12-31' } as any, mockUser);
    });

    it('should filter by budget range', async () => {
      await service.findAll({ budgetMin: 1000, budgetMax: 500000 } as any, mockUser);
    });

    it('should filter by propertyId and category', async () => {
      await service.findAll({ propertyId: 'prop-1', category: 'IT' } as any, mockUser);
    });

    it('should filter by VENDOR role via invitations', async () => {
      prisma.user.findUnique.mockResolvedValue({ id: 'vendor-user', propertyId: null });
      await service.findAll({} as any, { id: 'vendor-user', role: 'VENDOR' });
    });
  });
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException', async () => {
      ...
      await expect(service.findById('invalid', mockUser)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    it('should update a DRAFT procurement', async () => {
      prisma.procurement.findUnique.mockResolvedValue(mockProcurement);
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, title: 'Updated Title' });

      const result = await service.update('proc-1', { title: 'Updated Title' }, 'user-1');
      expect(result).toHaveProperty('title', 'Updated Title');
    });

    it('should reject updates to non-DRAFT procurement', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'SUBMITTED' });
    ...
    it('should return NotFound for missing procurement', async () => {
      prisma.procurement.findUnique.mockResolvedValue(null);
      await expect(service.update('invalid', { title: 'Test' }, 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('workflow transitions', () => {
    ...
    it('should submit from DRAFT to SUBMITTED', async () => {
      prisma.procurement.findUnique.mockResolvedValue(mockProcurement);
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'SUBMITTED', currentOwnerRole: 'REQUESTER' });
    ...
    it('should start review from SUBMITTED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'SUBMITTED', currentOwnerRole: 'REQUESTER' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'UNDER_PROCUREMENT_REVIEW', currentOwnerRole: 'PROCUREMENT' });
    ...
    it('should approve review from UNDER_PROCUREMENT_REVIEW', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'UNDER_PROCUREMENT_REVIEW', currentOwnerRole: 'PROCUREMENT' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'APPROVED', currentOwnerRole: 'PROCUREMENT' });

      const result = await service.approveReview('proc-1', 'user-1', 'Looks good');
      expect(result).toHaveProperty('status', 'APPROVED');
    });

    it('should return for revision from UNDER_PROCUREMENT_REVIEW', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'UNDER_PROCUREMENT_REVIEW' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RETURNED_FOR_REVISION' });

      const result = await service.returnReview('proc-1', 'user-1', 'Fix budget');
      expect(result).toHaveProperty('status', 'RETURNED_FOR_REVISION');
    });

    it('should reject review from UNDER_PROCUREMENT_REVIEW', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'UNDER_PROCUREMENT_REVIEW' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'REJECTED', currentOwnerRole: 'PROCUREMENT' });

      const result = await service.rejectReview('proc-1', 'user-1', 'Not suitable');
      expect(result).toHaveProperty('status', 'REJECTED');
    });

    it('should publish from APPROVED to RFP_PUBLISHED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'APPROVED' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RFP_PUBLISHED', currentOwnerRole: 'VENDOR' });

      const result = await service.publish('proc-1', 'user-1', '2026-08-01T00:00:00Z');
      expect(result).toHaveProperty('status', 'RFP_PUBLISHED');
    });

    it('should publish RFI from APPROVED to RFI_PUBLISHED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'APPROVED', requestType: 'RFI' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RFI_PUBLISHED', requestType: 'RFI', currentOwnerRole: 'VENDOR' });
    ...
    it('should publish RFQ from APPROVED to RFQ_OPEN', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'APPROVED', requestType: 'RFQ' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RFQ_OPEN', requestType: 'RFQ', currentOwnerRole: 'VENDOR' });
    ...
    it('should throw if procurement not found on publish', async () => {
      prisma.procurement.findUnique.mockResolvedValue(null);
      await expect(service.publish('invalid', 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('should cancel from any status', async () => {
      prisma.procurement.findUnique.mockResolvedValue(mockProcurement);
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'CANCELLED' });

      const result = await service.cancel('proc-1', 'user-1', 'No longer needed');
      expect(result).toHaveProperty('status', 'CANCELLED');
    });

    it('should close RFI from RFI_COLLECTING', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'RFI_COLLECTING' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RFI_CLOSED' });
    ...
    it('should draft RFP from RFI_CLOSED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'RFI_CLOSED' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'RFP_DRAFTING' });
    ...
    it('should complete vendor response from RFQ_OPEN', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'RFQ_OPEN' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'VENDOR_RESPONSE_IN_PROGRESS' });
    ...
    it('should start ebidding from VENDOR_RESPONSE_IN_PROGRESS', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'VENDOR_RESPONSE_IN_PROGRESS' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'EBIDDING_PREP' });
    ...
    it('should complete ebidding', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'EBIDDING_PREP' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'EVALUATION' });
    ...
    it('should complete evaluation and route to approver', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'EVALUATION' });
      ...
      expect(mockApprovalService.routeToApprover).toHaveBeenCalledWith('proc-1');
    });

    it('should reject invalid transition from completeEvaluation', async () => {
      prisma.procurement.findUnique.mockResolvedValue(mockProcurement);
      await expect(service.completeEvaluation('proc-1', 'user-1')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should approve procurement from PENDING_APPROVAL to AWARD_APPROVED', async () => {
      ...
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'AWARD_APPROVED', currentOwnerRole: 'APPROVER' });

      const result = await service.approveProcurement('proc-1', 'user-1', 'Approved');
      expect(result).toHaveProperty('status', 'AWARD_APPROVED');
    });

    it('should announce award from AWARD_APPROVED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'AWARD_APPROVED' });
      prisma.procurementResult.upsert.mockResolvedValue({});
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'AWARD_ANNOUNCED' });

      const result = await service.announceAward('proc-1', 'user-1', 'vendor-1', 'Winner!');
    ...
    it('should throw on announceAward if not AWARD_APPROVED', async () => {
      ...
      await expect(service.announceAward('proc-1', 'user-1', 'vendor-1', '')).rejects.toThrow(BadRequestException);
    });

    it('should complete from AWARD_ANNOUNCED to COMPLETED', async () => {
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'AWARD_ANNOUNCED' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'COMPLETED', currentOwnerRole: 'CLOSED' });
    ...
    it('should notify requester separately when not in notifyRoles', async () => {
      prisma.user.findMany.mockResolvedValue([]);
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, requesterId: 'requester-id' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'SUBMITTED', currentOwnerRole: 'REQUESTER' });
    ...
    it('should notify for APPROVED status to REQUESTER role', async () => {
      ...
      prisma.procurement.findUnique.mockResolvedValue({ ...mockProcurement, status: 'UNDER_PROCUREMENT_REVIEW', requesterId: 'other' });
      prisma.procurement.update.mockResolvedValue({ ...mockProcurement, status: 'APPROVED' });
  ...
  describe('getStats', () => {
    it('should return aggregated statistics', async () => {
      ...
      prisma.procurement.findMany.mockResolvedValue([{ createdAt: now, status: 'DRAFT', budgetEstimate: 100000 }]);
  ...
  describe('error cases', () => {
    it('should throw BadRequestException for missing procurement on cancel', async () => {
      prisma.procurement.findUnique.mockResolvedValue(null);
      await expect(service.cancel('invalid', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
});
