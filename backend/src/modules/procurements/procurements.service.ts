...
export class ProcurementsService {
  ...
  async create(dto: CreateProcurementDto, userId: string) {
    ...
    if (!propertyId) {
      const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { propertyId: true } });
    ...
    await this.appendTimeline(procurement.id, 'DRAFT_CREATED', 'REQUESTER', userId, {});
    await this.logAudit('procurements', procurement.id, 'CREATE', userId, 'REQUESTER', null, procurement);
  ...
  async findAll(dto: QueryProcurementDto, user: { id: string; role: string }) {
    ...
    if (dto.status) where.status = dto.status as ProcurementStatus;
    ...
    if (user.role === 'REQUESTER') {
      ...
      const requester = await this.prisma.user.findUnique({ where: { id: user.id }, select: { propertyId: true } });
      ...
      const approver = await this.prisma.user.findUnique({ where: { id: user.id }, select: { propertyId: true } });
    ...
    const allowedSorts = ['createdAt', 'title', 'requestNo', 'status', 'budgetEstimate', 'requestType'];
    ...
    return {
      data: procurements,
      meta: { page, limit, total, totalPages: Math.ceil(total / limit), statusCounts: counts },
    };
  }
  ...
  async findById(id: string, user: { id: string; role: string }) {
    const procurement = await this.prisma.procurement.findUnique({
      ...
      include: {
        ...
        property: true,
        department: true,
        files: { select: { id: true, fileName: true, mimeType: true, fileSize: true, createdAt: true } },
        timelines: { orderBy: { timestamp: 'desc' } },
        invitations: {
          include: { vendor: { select: { id: true, companyName: true, userId: true } } },
        },
        _count: { select: { submissions: true, ebiddingRounds: true, evaluatorAssignments: true } },
      },
    });
    ...
    if (user.role === 'VENDOR') {
      const invitation = procurement.invitations?.find(inv => inv.vendor?.userId === user.id);
      if (!invitation) throw new ForbiddenException('Access denied');
    } else if (user.role === 'REQUESTER') {
      if (procurement.requesterId !== user.id) throw new ForbiddenException('Access denied');
    }
  ...
  async update(id: string, dto: UpdateProcurementDto, userId: string) {
    ...
    if (!['DRAFT', 'RETURNED_FOR_REVISION'].includes(procurement.status)) {
      throw new BadRequestException('Can only update draft or returned procurements');
    }
    ...
    await this.appendTimeline(id, 'DRAFT_UPDATED', 'REQUESTER', userId, { fields: Object.keys(dto) });
    return updated;
  }
  ...
  async startReview(id: string, userId: string) {
    return this.transition(id, 'UNDER_PROCUREMENT_REVIEW', 'PROCUREMENT', userId, 'PROCUREMENT');
  }

  async approveReview(id: string, userId: string, comment?: string) {
    return this.transition(id, 'APPROVED', 'PROCUREMENT', userId, 'PROCUREMENT', { comment });
  }

  async returnReview(id: string, userId: string, reason?: string) {
    return this.transition(id, 'RETURNED_FOR_REVISION', 'PROCUREMENT', userId, 'PROCUREMENT', { reason });
  }

  async rejectReview(id: string, userId: string, reason?: string) {
    return this.transition(id, 'REJECTED', 'PROCUREMENT', userId, 'PROCUREMENT', { reason });
  }
  ...
  async publish(id: string, userId: string, submissionDeadline?: string) {
    ...
    if (!WORKFLOW_TRANSITIONS[procurement.status]?.includes(nextStatus)) {
      throw new BadRequestException(`Cannot publish from status ${procurement.status}`);
    }
    ...
    await this.appendTimeline(id, nextStatus, 'PROCUREMENT', userId, { submissionDeadline });
    await this.logAudit('procurements', id, 'PUBLISH', userId, 'PROCUREMENT', procurement, updated);

    return updated;
  }

  async cancel(id: string, userId: string, reason?: string) {
    return this.transition(id, 'CANCELLED', 'PROCUREMENT', userId, 'PROCUREMENT', { reason });
  }

  async closeRfi(id: string, userId: string) {
    return this.transition(id, 'RFI_CLOSED', 'PROCUREMENT', userId, 'PROCUREMENT');
  }

  async draftRfp(id: string, userId: string) {
    return this.transition(id, 'RFP_DRAFTING', 'PROCUREMENT', userId, 'PROCUREMENT');
  }

  async completeVendorResponse(id: string, userId: string) {
    return this.transition(id, 'VENDOR_RESPONSE_IN_PROGRESS', 'PROCUREMENT', userId, 'PROCUREMENT');
  }

  async startEbidding(id: string, userId: string) {
    return this.transition(id, 'EBIDDING_PREP', 'PROCUREMENT', userId, 'PROCUREMENT');
  }

  async completeEbidding(id: string, userId: string) {
    return this.transition(id, 'EVALUATION', 'PROCUREMENT', userId, 'PROCUREMENT');
  }
  ...
  async completeEvaluation(id: string, userId: string) {
    ...
    if (!allowedTransitions.includes('PENDING_APPROVAL')) {
      throw new BadRequestException(`Cannot transition from ${procurement.status} to PENDING_APPROVAL`);
    }

    const updated = await this.prisma.procurement.update({
      where: { id },
      data: { status: 'PENDING_APPROVAL', currentOwnerRole: 'APPROVER', currentStage: 'PENDING_APPROVAL' },
    });

    await this.appendTimeline(id, 'PENDING_APPROVAL', 'PROCUREMENT', userId, {});
    await this.logAudit('procurements', id, 'PENDING_APPROVAL', userId, 'PROCUREMENT', procurement, updated);
  ...
  async approveProcurement(id: string, userId: string, comment?: string) {
    return this.transition(id, 'AWARD_APPROVED', 'APPROVER', userId, 'APPROVER', { comment });
  }

  async announceAward(id: string, userId: string, winningVendorId: string, announcementText: string) {
    const procurement = await this.prisma.procurement.findUnique({ where: { id } });
    if (!procurement) throw new NotFoundException('Procurement not found');
    if (procurement.status !== 'AWARD_APPROVED') throw new BadRequestException('Must be award approved first');

    await this.prisma.procurementResult.upsert({
      where: { procurementId: id },
      create: { procurementId: id, winningVendorId, announcementText, announcedAt: new Date() },
      update: { winningVendorId, announcementText, announcedAt: new Date() },
    });

    return this.transition(id, 'AWARD_ANNOUNCED', 'PROCUREMENT', userId, 'PROCUREMENT', { winningVendorId });
  }
  ...
  private async transition(
    ...
    await this.appendTimeline(id, targetStatus, actorRole, userId, metadata || {});
    await this.logAudit('procurements', id, targetStatus, userId, actorRole, procurement, updated);
    ...
    switch (targetStatus) {
      case 'SUBMITTED': notifyRoles.push(UserRole.PROCUREMENT); break;
      case 'APPROVED': notifyRoles.push(UserRole.REQUESTER); break;
      case 'RETURNED_FOR_REVISION': notifyRoles.push(UserRole.REQUESTER); break;
      case 'REJECTED': notifyRoles.push(UserRole.REQUESTER); break;
      case 'RFP_PUBLISHED':
      case 'RFQ_OPEN':
      case 'RFI_PUBLISHED': notifyRoles.push(UserRole.VENDOR); break;
      case 'PENDING_APPROVAL': notifyRoles.push(UserRole.APPROVER, UserRole.ADMIN); break;
      case 'AWARD_APPROVED':
      case 'AWARD_ANNOUNCED':
      case 'COMPLETED': notifyRoles.push(UserRole.REQUESTER, UserRole.PROCUREMENT); break;
      default: notifyRoles.push(UserRole.ADMIN); break;
    }

    // Also notify the requester if they aren't the actor
    if (procurement.requesterId && procurement.requesterId !== userId && !notifyRoles.includes(UserRole.REQUESTER)) {
    ...
    if (notifyRoles.length > 0) {
      const targetUsers = await this.prisma.user.findMany({
        where: { role: { in: notifyRoles }, isActive: true, id: { not: userId } },
        select: { id: true },
      });
  ...
  async getStats() {
    const [total, byStatus, byType, byCategory, byMonth, recentActivity] = await Promise.all([
      ...
      this.prisma.procurement.groupBy({ by: ['category'], _count: true, where: { category: { not: null } } }),
      this.prisma.procurement.findMany({
      ...
      this.prisma.auditLog.findMany({
        ...
        select: { action: true, entityType: true, createdAt: true, actorRole: true },
      }),
    ]);

    const statusMap = Object.fromEntries(byStatus.map(s => [s.status, s._count]));
    const typeMap = Object.fromEntries(byType.map(t => [t.requestType, t._count]));
    const categoryMap = Object.fromEntries(byCategory.map(c => [c.category || 'Uncategorized', c._count]));
    ...
    const totalBudget = byMonth.reduce((sum, item) => sum + Number(item.budgetEstimate || 0), 0);
    ...
    return {
      ...
      byMonth: Object.entries(monthlyData).map(([month, data]) => ({ month, ...data })),
      recentActivity,
    };
  }
}
