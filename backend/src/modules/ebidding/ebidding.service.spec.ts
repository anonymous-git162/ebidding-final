...
describe('EbiddingService', () => {
  ...
  describe('createRound', () => {
    ...
    it('should increment round number for subsequent rounds', async () => {
      ...
      prisma.ebiddingRound.updateMany.mockResolvedValue({ count: 0 });
      prisma.ebiddingRound.create.mockResolvedValue({
        ...mockRound,
        roundNo: 3,
      });
  ...
  describe('openRound', () => {
    it('should open a pending round', async () => {
      ...
      prisma.ebiddingRound.updateMany.mockResolvedValue({ count: 0 });
      prisma.ebiddingRound.update.mockResolvedValue({
        ...mockRound,
        status: 'OPEN',
        startsAt: new Date(),
      });
    ...
    it('should reject opening a non-pending round', async () => {
      prisma.ebiddingRound.findUnique.mockResolvedValue({
        ...mockRound,
        status: 'CLOSED',
      });

      await expect(service.openRound('round-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('closeRound', () => {
    ...
    it('should reject closing a non-open round', async () => {
      ...
      await expect(service.closeRound('round-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('placeBid', () => {
    beforeEach(() => {
      ...
      prisma.vendorInvitation.findFirst.mockResolvedValue({
        id: 'inv-1',
    ...
    it('should place a new bid', async () => {
      ...
      prisma.ebiddingResponse.create.mockResolvedValue({
        id: 'bid-1',
        ebiddingRoundId: 'round-1',
        vendorId: 'vendor-1',
        bidAmount: 1000,
      });
    ...
    it('should update existing bid', async () => {
      prisma.ebiddingResponse.findFirst.mockResolvedValue({
        id: 'bid-1',
        vendorId: 'vendor-1',
        bidAmount: 800,
      });
      prisma.ebiddingResponse.update.mockResolvedValue({
        id: 'bid-1',
        bidAmount: 900,
      });
    ...
    it('should reject bids on non-open rounds', async () => {
      ...
      await expect(
        service.placeBid('round-1', 'vendor-user-1', 1000),
  ...
  describe('getRounds', () => {
    it('should return rounds with bid data for procurement role', async () => {
      ...
      const result = await service.getRounds('proc-1', {
        id: 'proc-user',
        role: 'PROCUREMENT',
      });
  ...
  describe('placeBid extra cases', () => {
    ...
    it('should reject bid from non-existent vendor', async () => {
      ...
      await expect(
        service.placeBid('round-1', 'unknown-user', 1000),
  ...
  describe('getMyBids', () => {
    it('should return bids for vendor in round', async () => {
      prisma.vendor.findUnique.mockResolvedValue(mockVendor);
      prisma.ebiddingResponse.findMany.mockResolvedValue([
        {
          id: 'b1',
          bidAmount: 500,
          vendorId: 'vendor-1',
    ...
    it('should throw for non-existent vendor', async () => {
      prisma.vendor.findUnique.mockResolvedValue(null);
      await expect(service.getMyBids('round-1', 'unknown')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('getAllMyBids', () => {
    ...
    it('should throw for non-existent vendor', async () => {
      prisma.vendor.findUnique.mockResolvedValue(null);
      await expect(service.getAllMyBids('unknown')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('deleteBid', () => {
    beforeEach(() => {
      prisma.vendor.findUnique.mockResolvedValue(mockVendor);
      prisma.ebiddingResponse.findUnique.mockResolvedValue({
        id: 'bid-1',
        vendorId: 'vendor-1',
        ebiddingRoundId: 'round-1',
      });
    ...
    it('should throw for non-existent vendor', async () => {
      prisma.vendor.findUnique.mockResolvedValue(null);
      await expect(service.deleteBid('bid-1', 'unknown')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw for non-existent bid', async () => {
      ...
      await expect(
        service.deleteBid('invalid', 'vendor-user-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw when bid belongs to another vendor', async () => {
      ...
      await expect(service.deleteBid('bid-1', 'vendor-user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
